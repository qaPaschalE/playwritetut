...
export function When_I_get_nth_element(position: number) {
  const playwrightElement = getPlaywrightElement();
  const index = position > 0 ? position - 1 : position;

  // Using Playwright's array-like behavior to select the nth element
  setPlaywrightElement(playwrightElement.nth(index));
}
